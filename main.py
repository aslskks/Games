...
def post_game_request():
    ...
    with sqlite3.connect(DB) as conn:
        ...
        cursor.execute(
            "INSERT INTO requests (name, link, message, created_at) VALUES (?, ?, ?, ?)",
            (game_name, game_link, message, created_at),
        )
        conn.commit()
...
def see():
    ...
    return render_template(
        "requests.html", data=requests_data, views=views_data, user_count=user_count
    )
...
def remove_request(index):
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": f"Failed to contact external server: {str(e)}"}
        )
...
def remove():
    try:
        ...
        with sqlite3.connect(DB) as conn:
            ...
            conditions = " AND ".join([f"{k} = ?" for k in row_to_remove.keys()])
...
def update_json():
    while True:
        ...
        full_data = {"requests": requests_data, "ips": ips_data, "views": views_data}
...
@app.get("/ovo/<path>")
@app.get("/ovo/<path:path>")
def ovo_path(path):
    return send_from_directory("templates/ovo/mobile/games/media", path)


@app.get("/media/<path:path>")
def media(path):
    path = path.replace("%20", " ")
    return send_from_directory("templates/ovo/mobile/games/ovo/media", path)
